...
## Como funciona
1. Build em Node 22 com `npm run build` → gera `dist/client` com o shell SPA (`_shell.html`)
2. Stage final usa **Node 22 Alpine** com `scripts/easypanel-server.mjs`
3. O servidor entrega os arquivos estáticos de `dist/client` e usa `_shell.html` como fallback para as rotas do site
...
- Confirme que o build gerou `dist/client/_shell.html`
